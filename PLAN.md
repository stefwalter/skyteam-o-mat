...
- **Embed config (built-in, not runtime).** The following are hardcoded in the JS at build time and do not change at runtime: (1) **trigger selector** = `a[href*=hike-fly-woche][class~=booking]` (links that open the wizard), (2) **result field selector** = `input#bemerkungen` (field that receives the stored result), (3) **storage key** = `"skyteam_o_mat"` (sessionStorage key for the wizard result). To change them, edit the constants in the source and rebuild.
- **Build: single JS file, no separate CSS.** The default build (`npm run build`), the embed build (`npm run build:embed`, output under `dist/`), and the WordPress bundle (`npm run build:wp`, output under `wordpress/skyteam-o-mat/assets/` plus a distributable zip) each produce **one JavaScript file** for the wizard/embed path. All CSS is injected at runtime by the JS (via a Vite plugin), so there is **no separate CSS file** next to that bundle. The host page only needs to include the one script (or activate the WordPress plugin, which enqueues it).
